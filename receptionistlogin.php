...
{
	echo "<script>window.location='receptionistaccount.php';</script>";
}
if(isset($_POST[submit]))
{
	$sql = "SELECT * FROM receptionist WHERE loginid='$_POST[loginid]' AND password='$_POST[password]' AND status='Active'";
	$qsql = mysqli_query($con,$sql);
	if(mysqli_num_rows($qsql) == 1)
	{
		$rslogin = mysqli_fetch_array($qsql);
		$_SESSION[receptionistid]= $rslogin[receptionistid] ;
		echo "<script>window.location='receptionistaccount.php';</script>";
	}
	else
	{
...
<div class="wrapper col2">
  <div id="breadcrumb">
    <ul>
      <li class="first">receptionist Login Panel</li></ul>
  </div>
</div>
